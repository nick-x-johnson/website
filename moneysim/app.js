(() => {
  ...
  function updateTimeline() {
    ...
    $('#timeline-values').innerHTML = ['pension', 'mortgage', 'interestSaved', 'investment'].map(key => `<div>${key === 'interestSaved' ? 'Mortgage interest saved' : series[key].label}<strong>${pounds(valueOf(row, key))}</strong></div>`).join('');
  }
  ...
  function exportProjection() {
    ...
    link.href = url; link.download = `moneysim-projection-${p.startDate}-${scenario}-${money}.csv`; document.body.append(link); link.click(); link.remove();
